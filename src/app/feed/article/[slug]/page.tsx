  ...
  if (!article) {
    return (
      <div className="min-h-screen bg-[#0a0a0a] text-white font-sans">
        <NavbarWrapper showSearch={false} showActions user={user} />
        ...
        <div className="flex items-center justify-between py-6 border-y border-white/10 mb-8">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="font-medium text-sm">{article.author_name}</span>
              <span className="text-xs text-white/50">
                Published {formatDate(article.created_at)} · {article.read_time}{" "}
                read
              </span>
            </div>
          </div>
